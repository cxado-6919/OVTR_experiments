...
class BDD(_BaseDataset):
    ...
    def get_preprocessed_seq_data_thr(self, raw_data, cls, assignment=None):
        ...
        for t in range(raw_data["num_timesteps"]):
            ...
            unmatched_indices = np.arange(raw_data['tk_ids'][t].shape[0])
            if raw_data['gt_ids'][t].shape[0] > 0 and raw_data['tk_ids'][t].shape[0] > 0:
                matching_scores = raw_data['similarity_scores'][t].copy()
                matching_scores[matching_scores < 0.5 - np.finfo('float').eps] = 0
                match_rows, match_cols = linear_sum_assignment(-matching_scores)
                actually_matched_mask = matching_scores[match_rows, match_cols] > 0 + np.finfo('float').eps
                match_cols = match_cols[actually_matched_mask]
                unmatched_indices = np.delete(unmatched_indices, match_cols, axis=0)
            unmatched_tracker_dets = raw_data['tk_dets'][t][unmatched_indices, :]
            ...
            raw_data['tk_ids'][t] = np.delete(raw_data['tk_ids'][t], to_remove_tracker, axis=0)
            raw_data['tk_dets'][t] = np.delete(raw_data['tk_dets'][t], to_remove_tracker, axis=0)
            raw_data['tk_classes'][t] = np.delete(raw_data['tk_classes'][t], to_remove_tracker, axis=0)
            raw_data['similarity_scores'][t] = np.delete(raw_data['similarity_scores'][t], to_remove_tracker, axis=1)
