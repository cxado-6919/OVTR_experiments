...
def _cv2_show_tracks(
    ...
    for bbox, label, id in zip(bboxes, labels, ids):
        ...
        # )
        # class tag
        class_tag = classes[label]
        img[
            y1 - 2 * text_height : y1 - text_height,
            x1 : x1 + len(class_tag) * text_width,
            :,
        ] = bbox_color
        cv2.putText(
            img,
...
def _plt_show_tracks(
    ...
    for bbox, label, id in zip(bboxes, labels, ids):
        ...
        plt.gca().add_patch(
            Rectangle(left_top, w, h, thickness, edgecolor=color, facecolor="none")
        )
